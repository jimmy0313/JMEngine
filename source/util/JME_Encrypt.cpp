...
namespace JMEngine
{
	namespace encrypt
	{
		...
		{   
			...
			for(i=0; i<18; )   
				Encrypt_4(block), m_auiP[i++] = block.m_uil, m_auiP[i++] = block.m_uir;   
			for(j=0; j<4; j++)   
				for(int k=0; k<256; )   
					Encrypt_4(block), m_auiS[j][k++] = block.m_uil, m_auiS[j][k++] = block.m_uir;   
		}   

		//Sixteen Round Encipher of Block   
		void CBlowFish::Encrypt_4(SBlock& block)   
		{   
		...
		void CBlowFish::Decrypt_4(SBlock& block)   
		{   
			unsigned int uiLeft = block.m_uil;   
			unsigned int uiRight = block.m_uir;   
			uiLeft ^= m_auiP[17];   
		...
		void CBlowFish::Encrypt_2(unsigned char* buf, size_t n, int iMode)   
		{   
		...
		//Returns false if n is multiple of 8   
		void CBlowFish::Decrypt_2(unsigned char* buf, size_t n, char x_char, int iMode)   
		{   
			//Check the buffer's length - should be > 0 and multiple of 8   
			if((n==0)||(n%8!=0))
				return;
		...
		//Returns false if n is multiple of 8   
		void CBlowFish::Encrypt_3(const unsigned char* in, unsigned char* out, size_t n, int iMode)   
		{   
			//Check the buffer's length - should be > 0 and multiple of 8   
			if((n==0)||(n%8!=0))
				return;
		...
		//Returns false if n is multiple of 8   
		void CBlowFish::Decrypt_3(const unsigned char* in, unsigned char* out, size_t n, int iMode)   
		{   
			//Check the buffer's length - should be > 0 and multiple of 8   
			if((n==0)||(n%8!=0))
				return;
		...
		void CBlowFish::Encrypt( const char* strInput, string& strOutput,const char* strKey,int bitCount, int len )   
		{   
			...
			oBlowFish.Encrypt_3((unsigned char*)strInput, (unsigned char*)pszDataOut, nNewLength);   
			pszDataOut[ nNewLength ]='\0';   

			std::string tempData(pszDataOut,nNewLength);
			strOutput = tempData;
		...
		void CBlowFish::Decrypt( const char* strInput, string& strOutput,const char* strKey,int bitCount, int len)   
		{   
			CBlowFish oBlowFish((unsigned char*)strKey, bitCount);         
			int nLength = len;   
			int nDifference = nLength % 8;   
			...
			char* pszDataOut = new char[ nNewLength + 1 ];   
			oBlowFish.Decrypt_3((unsigned char*)strInput, (unsigned char*)pszDataOut, nNewLength);   
			pszDataOut[ nNewLength ]='\0';   

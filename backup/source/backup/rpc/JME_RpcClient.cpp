...
namespace JMEngine
{
	namespace rpc
	{
		...
		JMEngine::rpc::JME_IPM_RPCClient::JME_IPM_RPCClientPtr JME_IPM_RPCClient::create()
		{
			return JMEngine::rpc::JME_IPM_RPCClient::JME_IPM_RPCClientPtr(new JME_IPM_RPCClient);
		}
		...
		bool JME_IPM_RPCClient::callRpcMethod( const char* method, const google::protobuf::Message* params, JME_IPM_RpcCallback::RpcHandler cb )
		{
			try
			{
				...
				JME_IPM_Rpc r(method, params);

				_cbs[r._methodId] = JME_IPM_RpcCallback::create(cb);
			}
			catch(const char* e)
			{
		...
		bool JME_IPM_RPCClient::callRpcMethod( const char* method, const google::protobuf::Message* params, JME_IPM_RpcCallback::RpcHandler cb, size_t dt, JME_IPM_RpcCallback::RpcDeadHandler dcb )
		{
			try
			{
				...
				_cbs[r._methodId] = JME_IPM_RpcCallback::create(shared_from_this(), cb, dt, dcb, r._methodId);
			}
			catch(const char* e)
			{
				LogE << "Call rpc function failed, error: " << e << LogEnd;
